...
def main():
    ...
    app_description = "FSP v1.0, Few Strokes Painter -- Painting from a Photograph with a Few Strokes"
    brush_dir = 'brush/'
    brush_extname = '.png'
    def_arg = {
    ...
    if input_extname == '.json':
        ...
        pt = painter(target, pyd_max_levels = pyd_max_levels, pyd_min_size = pyd_min_size)
        pt.paint(n_max = n_max, brush_path = brush_path, brush_inv = brush_inv, brush_masking = brush_masking,              
                 ...
                 min_thres_r = min_thres_r, min_decline = min_decline,            
                 verbose = verbose)
        if len(output_dir) > 0:
            output_dir += '/'
